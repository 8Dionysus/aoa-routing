...
ENUM_ESCAPE_VALUE = "__governance_runtime_not_allowed__"

GOVERNANCE_RUNTIME_CONTRACTS = (
    ('appeal_route_decision', 'appeal_route_decision_v1.json'),
    ('charter_change_route', 'charter_change_route_v1.json'),
    ('council_dispatch', 'council_dispatch_v1.json'),
    ('governance_route_signal', 'governance_route_signal_v1.json'),
    ('runtime_case_route_v1', 'runtime_case_route_v1.json'),
)

CONTRACT_PATH_OVERRIDES = {
    'appeal_route_decision': (
        'mechanics/governance/parts/appeal-and-veto/schemas/appeal_route_decision_v1.json',
        'mechanics/governance/parts/appeal-and-veto/examples/appeal_route_decision.example.json',
    ),
    'charter_change_route': (
        'mechanics/governance/parts/charter-change-routing/schemas/charter_change_route_v1.json',
        'mechanics/governance/parts/charter-change-routing/examples/charter_change_route.example.json',
    ),
    'council_dispatch': (
        'mechanics/governance/parts/council-dispatch/schemas/council_dispatch_v1.json',
        'mechanics/governance/parts/council-dispatch/examples/council_dispatch.example.json',
    ),
    'governance_route_signal': (
        'mechanics/governance/parts/governance-escalation/schemas/governance_route_signal_v1.json',
        'mechanics/governance/parts/governance-escalation/examples/governance_route_signal.example.json',
    ),
    'runtime_case_route_v1': (
        'mechanics/titan/parts/runtime-case-routing/schemas/runtime_case_route_v1.json',
        'mechanics/titan/parts/runtime-case-routing/examples/runtime_case_route_v1.example.json',
    ),
}
...
def load_contract(stem: str, schema_file: str) -> tuple[dict[str, object], dict[str, object]]:
    schema_ref, example_ref = CONTRACT_PATH_OVERRIDES.get(
        stem,
        (f"schemas/{schema_file}", f"examples/{stem}.example.json"),
    )
...
class GovernanceRuntimeContractTests(unittest.TestCase):
    ...
    def test_governance_runtime_examples_match_schemas(self) -> None:
        self.assertTrue(GOVERNANCE_RUNTIME_CONTRACTS)
        missing_pairs: list[str] = []
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema_ref, example_ref = CONTRACT_PATH_OVERRIDES.get(
                stem,
                (f"schemas/{schema_file}", f"examples/{stem}.example.json"),
            )
        ...
        self.assertFalse(missing_pairs, "missing governance/runtime contract pair(s): " + ", ".join(missing_pairs))
    ...
    def test_governance_runtime_schemas_reject_unknown_fields(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance/runtime object fields were exercised")

    def test_governance_runtime_schemas_reject_wrong_types_for_every_field(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance/runtime fields were exercised")

    def test_governance_runtime_schemas_reject_missing_required_fields(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance/runtime required fields were exercised")

    def test_governance_runtime_schemas_reject_bad_array_items(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance/runtime array fields were exercised")

    def test_governance_runtime_schemas_reject_const_escapes(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance/runtime const fields were exercised")

    def test_governance_runtime_schemas_reject_enum_escapes(self) -> None:
        exercised = 0
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
        ...
        self.assertGreater(exercised, 0, "no governance/runtime enum fields were exercised")

    def test_governance_runtime_schemas_reject_invalid_numeric_ranges(self) -> None:
        for stem, schema_file in GOVERNANCE_RUNTIME_CONTRACTS:
            schema, example = load_contract(stem, schema_file)
            for path, value in walk_values(example):
