...
This card applies to `mechanics/` and every nested mechanic route until a
...
- Head mechanics use OS Abyss names: Agon is Agon, Experience is Experience,
  Checkpoint is Checkpoint, Recurrence is Recurrence.
- Parts are operation nodes, not file-type buckets.
- Payload docs, schemas, examples, config, manifests, generated companions,
  scripts, and tests move into the nearest owning part when that part owns the
  operation.
...
Report parent mechanics touched, active parts changed, payload homes moved,
legacy/provenance accounting updated, root compatibility surfaces preserved or
retired, and checks run.
