...
def load_builder():
    path = PART_ROOT / "scripts" / "build_agon_gate_routing_registry.py"
...
def load_validator():
    path = PART_ROOT / "scripts" / "validate_agon_gate_routing.py"
...
def test_agon_gate_registry_is_current():
    ...
    config = load_json(PART_ROOT / "config" / "agon_gate_routing.config.json")
    expected = json.dumps(
    ...
    current = (PART_ROOT / "generated" / "agon_gate_routing_registry.min.json").read_text(encoding="utf-8")
    assert current == expected


def test_agon_gate_registry_keeps_routing_thin():
    registry = load_json(PART_ROOT / "generated" / "agon_gate_routing_registry.min.json")
...
def test_agon_gate_validator_passes_current_registry():
    result = subprocess.run(
        [sys.executable, str(PART_ROOT / "scripts" / "validate_agon_gate_routing.py")],
        cwd=REPO_ROOT,
        text=True,
        capture_output=True,
        check=False,
