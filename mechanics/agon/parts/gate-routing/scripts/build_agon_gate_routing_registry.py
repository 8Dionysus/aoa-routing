...
OUTPUT_PATH = PART_ROOT / "generated" / "agon_gate_routing_registry.min.json"
...
def build_registry(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "routing_contour": config["routing_contour"],
...
def main() -> int:
    ...
    if args.check:
        ...
        if current != rendered:
            raise SystemExit(
                "mechanics/agon/parts/gate-routing/generated/"
                "agon_gate_routing_registry.min.json is stale"
            )
