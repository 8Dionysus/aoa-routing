...
REGISTRY_PATH = PART_ROOT / "generated" / "agon_gate_routing_registry.min.json"
CONFIG_PATH = PART_ROOT / "config" / "agon_gate_routing.config.json"
SCHEMA_NAME = "agon-gate-routing-registry.schema.json"
CENTER_LAWFUL_MOVE_REGISTRY_PATH = (
    REPO_ROOT.parent / "Agents-of-Abyss" / "generated" / "agon_lawful_move_registry.min.json"
)
...
def load_schema(schema_name: str) -> dict[str, Any]:
    return load_json(PART_ROOT / "schemas" / schema_name)


@lru_cache(maxsize=None)
def get_schema_registry() -> Registry:
    resources: list[tuple[str, Resource]] = []
    for schema_path in sorted((PART_ROOT / "schemas").glob("*.schema.json")):
...
def optional_dependency_report() -> list[str]:
    reports: list[str] = []
    center_root = REPO_ROOT.parent / "Agents-of-Abyss"
    ...
    formation = REPO_ROOT.parent / "aoa-agents" / "generated" / "agent_formation_trial.min.json"
    ...
    if owner.exists():
        ...
        reports.append("optional owner binding registry not found")
...
def main() -> int:
    subprocess.run(
        [sys.executable, str(PART_ROOT / "scripts" / "build_agon_gate_routing_registry.py"), "--check"],
        check=True,
    )
