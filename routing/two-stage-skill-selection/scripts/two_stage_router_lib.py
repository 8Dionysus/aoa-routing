...
def is_negated_phrase(task_normalized: str, normalized_phrase: str) -> bool:
    ...
    patterns = [
        ...
        rf"\bbefore (?:writing|write|creating|create|recording|record)(?: [a-z0-9]+){{0,6}} {escaped_phrase}\b",
        rf"\binstead of (?:writing|write|creating|create|recording|record)(?: [a-z0-9]+){{0,6}} {escaped_phrase}\b",
    ]
...
def merge_top_bands_with_shortlist(
    top_bands: list[dict[str, Any]],
    shortlist: list[dict[str, Any]],
    *,
    max_band_candidates: int,
) -> list[dict[str, Any]]:
    inferred_bands = infer_top_bands_from_shortlist(
        shortlist,
        max_band_candidates=max_band_candidates,
    )
    by_id = {entry["id"]: entry for entry in top_bands}
    for inferred in inferred_bands:
        current = by_id.get(inferred["id"])
        if current is None or int(inferred["score"]) > int(current["score"]):
            by_id[inferred["id"]] = inferred
    ranked_bands = sorted(
        by_id.values(),
        key=lambda entry: (-int(entry["score"]), entry["id"]),
    )
...
def preselect(
    ...
    *,
    ...
    if shortlist:
        top_bands = merge_top_bands_with_shortlist(
            top_bands,
            shortlist,
