#!/usr/bin/env python3
"""Validate two-stage skill-routing artifacts."""
...
def resolve_routing_root(path: Path) -> tuple[Path, bool]:
    ...
    for candidate in (resolved, *resolved.parents):
        if (candidate / POLICY_PATH).exists():
            return candidate, False
    return Path(__file__).resolve().parents[3], True
...
def validate_outputs(routing_root: Path, skills_root: Path) -> list[tuple[str, str]]:
    ...
    if policy.get("defaults", {}).get("fallback_visibility_mode") != "out_of_band_only":
        issues.append((POLICY_PATH.as_posix(), "fallback_visibility_mode must stay 'out_of_band_only'"))
    ...
    if skill_root_starter not in starter_names:
        issues.append(("tiny_model_entrypoints.json", f"missing starter {skill_root_starter!r} required by two-stage policy"))
    ...
    if prompt_blocks.get("policy_ref") != POLICY_PATH.as_posix():
    ...
    if tool_schemas.get("policy_ref") != POLICY_PATH.as_posix():
    ...
    if manifest.get("policy_ref") != POLICY_PATH.as_posix():
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate two-stage skill-routing artifacts.")
