...
TWO_STAGE_SCHEMA_ROOT = "routing/two-stage-skill-selection/schemas"
FIXTURE_POLICY_PATH = (
    FIXTURES_ROOT
    / "aoa-routing"
    / "routing"
...
def test_build_outputs_include_two_stage_router_surfaces() -> None:
    ...
    assert outputs["two_stage_skill_entrypoints.json"]["schema_ref"] == (
        f"{TWO_STAGE_SCHEMA_ROOT}/two-stage-skill-entrypoints.schema.json"
    )
...
def test_live_workspace_two_stage_outputs_are_normalized_v2() -> None:
    ...
    assert entrypoints["schema_ref"] == f"{TWO_STAGE_SCHEMA_ROOT}/two-stage-skill-entrypoints.schema.json"
    ...
    assert prompt_blocks["schema_ref"] == f"{TWO_STAGE_SCHEMA_ROOT}/two-stage-router-prompt-blocks.schema.json"
    ...
    assert tool_schemas["schema_ref"] == f"{TWO_STAGE_SCHEMA_ROOT}/two-stage-router-tool-schemas.schema.json"
    ...
    assert examples["schema_ref"] == f"{TWO_STAGE_SCHEMA_ROOT}/two-stage-router-examples.schema.json"
    ...
    assert manifest["schema_ref"] == f"{TWO_STAGE_SCHEMA_ROOT}/two-stage-router-manifest.schema.json"
