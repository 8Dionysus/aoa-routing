...
two-stage skill selection adds an optional adjacent routing seam for small-model skill selection.
...
`routing/two-stage-skill-selection/config/two_stage_router_precision_cases.jsonl` is the routing-owned local
...
two-stage skill selection does not replace `generated/tiny_model_entrypoints.json` or the existing inspect/expand/pair/recall path.
