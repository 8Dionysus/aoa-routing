...
INDEX_PATHS = (
    ...
    INDEX_DIR / "by-routing-surface.md",
...
def collect_decision_records(repo_root: Path = REPO_ROOT) -> tuple[list[DecisionRecord], list[tuple[str, str]]]:
    ...
    for path in sorted(decision_root.glob("*.md")):
        ...
        if not path.name.startswith(f"{DECISION_PREFIX}-"):
            issues.append((rel, "top-level decision markdown must use AOA-RT-D-####-slug.md"))

    for path in sorted(decision_root.glob(f"{DECISION_PREFIX}-*.md")):
        rel = path.relative_to(repo_root).as_posix()
        match = DECISION_RE.match(path.name)
        if not match:
            issues.append((rel, "filename must match AOA-RT-D-####-slug.md"))
            continue
        ...
        routing_surfaces = _metadata_value(metadata, "Routing surfaces", rel, issues)
        ...
        if decision_id and original_date and title:
            records.append(
                DecisionRecord(
                    ...
                    routing_surfaces=_split_values(routing_surfaces),
    ...
    if numbers:
        ...
        if numbers != expected_numbers:
            issues.append((DECISIONS_DIR.as_posix(), "decision numbers must be contiguous from AOA-RT-D-0001"))
...
def validate_index_contract(repo_root: Path = REPO_ROOT) -> list[tuple[str, str]]:
    ...
    text = path.read_text(encoding="utf-8")
    required = [
        "schema_version: aoa_rt_decision_index_contract_v1",
        "authority: docs/decisions/AGENTS.md",
        "source_records: docs/decisions/AOA-RT-D-*.md",
...
def _contract_list_values(repo_root: Path, key: str, issues: list[tuple[str, str]]) -> set[str]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if in_list and line and not line.startswith(" "):
            break
    if not key_seen:
        issues.append(((INDEX_DIR / "index_contract.yaml").as_posix(), f"{key} must be a top-level list; use [] when empty"))
    elif not values:
        issues.append(((INDEX_DIR / "index_contract.yaml").as_posix(), f"{key} must be a list, not null; use [] when empty"))
    return values
...
def _table(records: list[DecisionRecord], *, columns: tuple[str, ...]) -> str:
    ...
    for record in records:
        ...
        for column in columns:
            ...
            elif column == "Routing surfaces":
                values.append(", ".join(record.routing_surfaces))
...
def render_index_files(records: list[DecisionRecord]) -> dict[Path, str]:
    ...
    return {
        INDEX_DIR / "README.md": (
            ...
            "- [By routing surface](by-routing-surface.md)\n"
        ...
        INDEX_DIR / "by-routing-surface.md": _grouped_index(
            "Decisions By Routing Surface",
            _group_by(records, "routing_surfaces"),
        ),
