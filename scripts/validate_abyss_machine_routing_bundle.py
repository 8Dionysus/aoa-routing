...
def _registry_roundtrip(
    ...
    *,
    ...
    registered = artifact_bundles.write_bundle_registry_record(
        ...
        consumer_refs=["aoa-routing:thin-router-readmodel"],
...
def _verify_materialized_subject_store(
    ...
    materialized = artifact_bundles.materialize_artifact_subjects(
        ...
        expected_source_repo="aoa-routing",
    )
...
def validate_bundle(
    ...
    *,
    ...
    if bundle_dir is not None:
        target_registry = registry_dir or bundle_dir.parent / "thin-router-readmodel-registry"
        return _validate_in_bundle_dir(manifest, subject, bundle_dir, target_registry, clean=clean)

    tmp_root = _default_tmp_root()
    with tempfile.TemporaryDirectory(prefix="aoa-routing-thin-router-bundle-", dir=tmp_root) as tmp:
        target = Path(tmp) / "thin-router-readmodel"
        target_registry = Path(tmp) / "thin-router-readmodel-registry"
