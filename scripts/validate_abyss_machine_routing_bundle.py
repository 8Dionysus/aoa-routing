...
DEFAULT_MANIFEST = REPO_ROOT / "docs" / "artifact-bundles" / "thin_router.bundle.json"
DEFAULT_SUBJECT = REPO_ROOT / "generated" / "aoa_router.min.json"
DEFAULT_BUNDLE_DIR = REPO_ROOT / "dist" / "abyss-artifact-bundle" / "aoa-routing-thin-router"
DEFAULT_REGISTRY_DIR = REPO_ROOT / "dist" / "abyss-artifact-registry" / "aoa-routing-thin-router"
DEFAULT_SUBJECT_STORE_ROOT = REPO_ROOT / "dist" / "abyss-artifact-subjects" / "aoa-routing-thin-router"
ARTIFACT_CLASS = "thin_routing_readmodel_bundle"
OWNER_REPO = "aoa-routing"
CONSUMER_INTENT = "agent"
CONSUMER_REF = "aoa-routing:thin-router-readmodel"
...
def _public_string_ref(value: str, abyss_machine_root: Path | None) -> str:
    ...
    if value == str(repo_root) or value.startswith(str(repo_root) + os.sep):
        return _portable_ref(Path(value))
    if abyss_machine_root is not None:
        abyss_root = abyss_machine_root.resolve()
        if value == str(abyss_root):
            return "repo:abyss-machine"
        if value.startswith(str(abyss_root) + os.sep):
            suffix = Path(value).resolve().relative_to(abyss_root).as_posix()
...
def _assert_manifest_matches_subject(manifest: Path, subject: Path) -> None:
    ...
    if payload.get("owner_repo") != "aoa-routing":
        raise ValueError("manifest owner_repo must be aoa-routing")
...
def _assert_public_safe_subjects(manifest: Path, subject: Path) -> None:
    ...
    if leaks:
        raise ValueError("thin routing bundle subjects contain private or machine-local markers: " + ", ".join(leaks))
...
def _assert_expected_controls(artifact_bundles: Any, bundle_dir: Path, verify: dict[str, Any], identity: dict[str, Any]) -> None:
    ...
    if not (bundle_dir / artifact_bundles.SBOM_CYCLONEDX_SIDECAR).exists():
        raise ValueError("thin routing bundle must emit SBOM-lite subject inventory")
...
def _verify_missing_sbom_control(
    ...
    for sidecar_name in (artifact_bundles.SBOM_CYCLONEDX_SIDECAR, artifact_bundles.SBOM_SPDX_SIDECAR):
        path = candidate / sidecar_name
        if path.exists():
            path.unlink()
    verification = artifact_bundles.verify_bundle(candidate, repo_root=abyss_repo_root)
...
def _verify_private_readmodel_marker(manifest: Path, subject: Path, tmp_root: Path) -> dict[str, Any]:
    candidate = tmp_root / "private-aoa-router.min.json"
    ...
    try:
        ...
        return {
            "ok": "thin routing bundle subjects contain private or machine-local markers" in str(exc),
            "error": str(exc),
        }
    return {"ok": False, "error": "private routing readmodel marker was not detected"}
...
def _verify_terminal_registry_state(
    ...
    revoked = artifact_bundles.write_bundle_registry_record(
        ...
        revocation_reason="aoa-routing thin router terminal-state rehearsal",
        source_repo=OWNER_REPO,
        source_ref=_portable_ref(manifest),
        producer=PRODUCER,
        trust_root_mode=TRUST_ROOT_MODE,
...
def _run_adversarial_checks(
    ...
    with tempfile.TemporaryDirectory(prefix="aoa-routing-thin-router-negative-", dir=_default_tmp_root()) as tmp:
        tmp_root = Path(tmp)
        checks = {
            ...
            "missing_sbom": _verify_missing_sbom_control(artifact_bundles, abyss_repo_root, bundle_dir, tmp_root),
            "wrong_external_subject": _verify_wrong_external_subject(artifact_bundles, abyss_repo_root, bundle_dir, tmp_root),
            "private_readmodel_marker": _verify_private_readmodel_marker(manifest, subject, tmp_root),
...
def _validate_in_bundle_dir(
    ...
    *,
    ...
    producer_command = "python scripts/validate_abyss_machine_routing_bundle.py"
    ...
    payload = {
        ...
        "schema": "aoa_routing_abyss_machine_thin_router_artifact_bundle_validation_v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate aoa-routing thin router through OS Abyss artifact bundles."
    )
    ...
    if args.json:
        ...
        print(
            "[ok] abyss-machine aoa-routing thin router artifact bundle verified: "
